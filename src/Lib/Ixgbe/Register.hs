...
waitClear reg mask = do
    logLn $ "Waiting for flags 0x" <> T.pack (showHex mask "") <> " in register " <> show reg <> " to clear."
    waitUntil reg mask (== 0)

waitSet :: (MonadIO m, MonadReader env m, Logger env, DeviceState m) => Register -> Word32 -> m ()
waitSet reg mask = do
    logLn $ "Waiting for flags 0x" <> T.pack (showHex mask "") <> " in register " <> show reg <> " to be set."
